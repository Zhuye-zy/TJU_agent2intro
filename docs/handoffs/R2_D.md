...
|D03|新增严格 KnowledgeRecord facts.json；只统计其201条独立事实，旧7复合摘要保留检索但不计入原子事实。来源实际去重为15；坐标计数要求来源、核验日、质量和实体状态。|原子记录/分校区计数一致，资料版本来自实际文件；坐标0。|
...
- 201条原子事实；旧摘要7；检索 Source 视图208；全文chunk 0；来源URL15。追加7个官方或校属设计院页面的72条不同建筑、文化、服务事实，达到200目标；未重复名称或chunk计数。
...
- ≥80%真实经纬度、每校区2张可用照片、现场每校区10点/3条路线仍未达成。200事实已达到；照片下载明确受403/429阻塞，详情见追加核验。
- 没有新增依赖或共享契约变更。增加 `get_assets` 同义方法兼容C，原路由 `get_campus_assets` 保持。

## 追加D03/D04复验（基于82e8794）

- 数据版本 `sha256:d75f2e7e8ac0`；201事实、106POI、15知识来源。相同固定50题重跑：40证据Recall@5=87.5%，其余10/10。36模型+知识测试通过。
- 新资料涉及第九楼、卫津东门/北洋广场/纪念亭、王学仲研究所、四湖、北洋校训石/宣怀广场/求实会堂/大通中心及场馆服务。逐条Source含真实URL与已知发布日期；未确定时间留null。
- 第九楼1953初建/1954竣工按来源和阶段区分；大活年份和面积来源不一致，没有强行选值；黾园机构成立和建筑落成分别记。台账：data/knowledge/SOURCE_DISCREPANCIES.md。
- M曾读取未跟踪的旧maps.py初稿提出北洋图层缩放疑问；提交82e8794中的SVG已统一原图→1000x800映射，M直接读已提交SVG后确认并撤回该疑问。本增量没有重跑旧初稿脚本。
- 大通中心身份通过2016校方文章“主轴线尽端、临水半岛、三个矩形体块”与2017图面相互印证，明确为身份对应推断，不是当前现场定位。
- 3张真实照片的文件说明/作者/CC BY-SA4.0许可已核验，见data/knowledge/media_candidates.json：卫津第九楼和青年湖（Amazingloong，2015-08-04）、北洋校区（Iamcannon，2019-11-24）。本机Commons文件页403、两条upload原图下载429；停止重试，没有换代理绕过，没有取得本地原图，因此未做图像目视核验，也没有放入可显示manifest。
- Commons分类中的“Tianjin University Beiyang Campus.jpg”实际上是地图，排除；附近地铁/南开照片排除。北洋第二张许可实景照片仍未找到，不以同图裁剪或地图凑数量。
- 所有校园JPG本地文件仍为0，assets.media仍空。当前可显示媒体依然0；不存在“有许可=已下载/已渲染”的假成功。
- 无高德请求、无地图配额消耗，模型密钥未读取。此次追加只有D数据、数据测试与本handoff；可合并后由M复核。
