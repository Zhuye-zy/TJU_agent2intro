...
## M1-R2 C04 追加返修（游览建议真实召回、正文上限）

基线：478db12，原 C 工作树安全 fast-forward 同步候选集成。责任范围仅 backend/model/service.py、stream_routes.py 与 tests/model/test_r2_stream.py，本节为新增交接。

根因：visit_plan 的泛化要求不包含具体实体名，原关键词检索可得 0 条，即使本地已存在多个点位；真实模型因此只能拒绝编计划。修复：仅对 content_generation + visit_plan，从 D 原有 list_pois/get_poi 获取有界目录候选；所选实体优先，其次用户关键词匹配，再补图书馆/文化/校门类别。最多读取 29 条目录候选，每实体按名称/最多两个别名 search，校区与 verified 状态严格过滤，去重后仍最多 5 个 Source、12000 字片段。资料直接复用 D，未创建第二套点位库。系统约束强调参观顺序不等于步行规划，不得编造距离、时长、门禁、开放时间和道路通行性。

本地真实数据复查（未发起模型请求）：
- 卫津路：5 个来源，春水馆、科学馆、爱晚湖、北洋大学堂纪念碑亭、北洋广场。
- 北洋园：5 个来源，郑东馆、校友林、春晖园、恢复高考纪念墙、海棠堤。
- 校园问答仍用原检索，未引入游览目录回退；带选中实体的来源排列经测试优先匹配该实体。

正文边界：23000 上限包含六字“【创作内容】”前缀；流式超限保留允许范围内的正文，随后发 INCOMPLETE_OUTPUT / length / partial=true；非流式超限明确失败，不产生超长成功结果。

验证：.venv/Scripts/python.exe -m pytest tests/model tests/maps -q → 32 passed；git diff --check 通过。新增真实本地双校区、多点位、选中实体优先与 campus_qa 不受影响用例；流式/非流式各测含前缀恰好 23000 与超限 1 字。其余 C01–C03 回归继续通过。
未验证：本次没有真实模型或高德请求，M 仍须合并后以真实 visit_plan 两样本及页面结果复验，不能将本地检索/隔离模型测试计为生成实测通过。
已停止修改，可以合并：是。
