# 参考图拍摄清单

状态：已覆盖 23 / 106 个点位；缺 83 个（详见 MISSING.md）。
图片命名 `<poi_id>-N.<ext>`，由脚本自动关联，manifest.csv 可查来源。

## 卫津路校区（50）

### 教学/科研
- [ ] **曾宪梓楼（25教）** `weijinlu-25-teaching` P1
- [ ] **水利馆** `weijinlu-water-building` P1
- [ ] **第九教学楼** `weijinlu-09-teaching` P1

### 图书馆
- [ ] **春水图书馆（北馆）** `weijinlu-chunshui-library` P1
- [ ] **科学图书馆（南馆）** `weijinlu-science-library` P1

### 校门
- [ ] **卫津路校区东门** `weijinlu-east-gate` P1

### 餐饮
- [ ] **学五食堂** `weijinlu-xue-5-dining` P2
- [ ] **学四食堂** `weijinlu-xue-4-dining` P1
- [ ] **留园餐厅** `weijinlu-liuyuan-dining` P2

### 宿舍区
- [ ] **一斋** `weijinlu-dorm-yi` P3
- [ ] **三斋** `weijinlu-dorm-san` P3
- [ ] **二斋** `weijinlu-dorm-er` P3
- [ ] **五斋** `weijinlu-dorm-wu` P3
- [ ] **六斋** `weijinlu-dorm-liu` P3
- [ ] **四斋** `weijinlu-dorm-si` P3

### 体育
- [ ] **体育场** `weijinlu-stadium` P1
- [ ] **体育馆** `weijinlu-gym` P1
- [ ] **游泳馆** `weijinlu-swimming` P1
- [ ] **老体育馆（篮球馆）** `weijinlu-old-gym` P1

### 文化/景观
- [x] **牛顿苹果树** `weijinlu-newton-tree` P1 ×2
- [ ] **东门广场** `weijinlu-east-gate-square` P1
- [ ] **冯骥才文学艺术研究院** `weijinlu-feng-jicai` P1
- [ ] **刘豹像** `weijinlu-liu-bao` P1
- [ ] **北洋大学堂纪念碑亭** `weijinlu-beiyang-pavilion` P1
- [ ] **北洋广场** `weijinlu-beiyang-square` P1
- [ ] **卢绳像** `weijinlu-lu-sheng` P1
- [ ] **友谊湖** `weijinlu-youyi-lake` P1
- [ ] **天工石** `weijinlu-tiangong-stone` P1
- [ ] **天津大学校史博物馆** `weijinlu-history-museum` P1
- [ ] **张太雷像** `weijinlu-zhang-tailei` P1
- [ ] **徐中像** `weijinlu-xu-zhong` P1
- [ ] **敬业湖** `weijinlu-jingye-lake` P1
- [ ] **敬业石** `weijinlu-jingye-stone` P1
- [ ] **李曙森像** `weijinlu-li-shusen` P1
- [ ] **校歌** `weijinlu-school-song` P1
- [ ] **梦成真石** `weijinlu-dream-stone` P1
- [ ] **求是亭** `weijinlu-qiushi-pavilion` P1
- [ ] **沈玉麟像** `weijinlu-shen-yulin` P1
- [ ] **溯源石** `weijinlu-suyuan-stone` P1
- [ ] **爱晚湖** `weijinlu-aiwan-lake` P1
- [ ] **王学仲艺术研究所** `weijinlu-wang-xuezhong` P1
- [ ] **盛宣怀像** `weijinlu-sheng-xuanhuai` P1
- [ ] **钦字第一号考凭** `weijinlu-qin-exam` P1
- [ ] **铭德道海棠** `weijinlu-haitang` P1
- [ ] **锡良模型园** `weijinlu-xiliang-garden` P1
- [ ] **青年湖** `weijinlu-youth-lake` P1
- [ ] **青年湖畔双亭** `weijinlu-double-pavilions` P1
- [ ] **马寅初像** `weijinlu-ma-yinchu` P1

### 服务
- [ ] **大学生活动中心** `weijinlu-student-center` P1
- [ ] **校友之家** `weijinlu-alumni-home` P1

## 北洋园校区（56）

### 教学/科研
- [x] **北洋园校区主楼** `beiyangyuan-main-building` P1 ×3
- [x] **国家大型地震工程模拟研究设施** `beiyangyuan-earthquake-facility` P1 ×3
- [x] **杏荪楼** `beiyangyuan-xingsun-building` P1 ×3
- [ ] **矩方规圆楼** `beiyangyuan-jufang-guiyuan` P1

### 图书馆
- [x] **郑东图书馆** `beiyangyuan-zhengdong-library` P1 ×3

### 校门
- [x] **北洋园校区东门** `beiyangyuan-east-gate` P1 ×2

### 餐饮
- [x] **学一食堂（梅园餐厅）** `beiyangyuan-xue-1-dining` P2 ×2
- [x] **学三食堂（棠园餐厅）** `beiyangyuan-xue-3-dining` P1 ×3
- [x] **学二食堂（兰园餐厅）** `beiyangyuan-xue-2-dining` P2 ×2
- [x] **学五食堂（桃园餐厅）** `beiyangyuan-xue-5-dining` P2 ×1
- [x] **学六食堂（菊园餐厅）** `beiyangyuan-xue-6-dining` P2 ×1
- [x] **学四食堂（竹园餐厅）** `beiyangyuan-xue-4-dining` P2 ×1
- [ ] **留学生食堂** `beiyangyuan-international-dining` P2

### 宿舍区
- [ ] **修园** `beiyangyuan-dorm-xiu` P3
- [ ] **平园** `beiyangyuan-dorm-ping` P3
- [ ] **格园** `beiyangyuan-dorm-ge` P3
- [ ] **正园** `beiyangyuan-dorm-zheng` P3
- [ ] **治园** `beiyangyuan-dorm-zhi-guo` P3
- [ ] **知园** `beiyangyuan-dorm-zhi` P3
- [ ] **诚园** `beiyangyuan-dorm-cheng` P3
- [ ] **齐园** `beiyangyuan-dorm-qi` P3

### 体育
- [x] **综合体育馆** `beiyangyuan-gym` P1 ×2

### 文化/景观
- [x] **三问桥** `beiyangyuan-sanwen-bridge` P1 ×2
- [x] **北洋纪念亭** `beiyangyuan-memorial-pavilion` P1 ×2
- [x] **天津大学星** `beiyangyuan-tju-star` P1 ×1
- [x] **实事求是校训石** `beiyangyuan-motto-stone` P1 ×1
- [x] **宣怀广场** `beiyangyuan-xuanhuai-square` P1 ×2
- [x] **尚贤石** `beiyangyuan-shangxian-stone` P1 ×1
- [x] **恢复高考纪念墙** `beiyangyuan-gaokao-wall` P1 ×2
- [x] **求实会堂** `beiyangyuan-qiushi-hall` P1 ×3
- [x] **苏州亭** `beiyangyuan-suzhou-pavilion` P1 ×3
- [ ] **书田广场** `beiyangyuan-shutian-square` P1
- [ ] **天麟广场** `beiyangyuan-tianlin-square` P1
- [ ] **太雷广场** `beiyangyuan-tailei-square` P1
- [ ] **春晖园** `beiyangyuan-chunhui-garden` P1
- [ ] **校友林景观带** `beiyangyuan-alumni-forest` P1
- [ ] **校训鼎** `beiyangyuan-motto-ding` P1
- [ ] **桃花堤** `beiyangyuan-peach-bank` P1
- [ ] **求是大道** `beiyangyuan-qiushi-avenue` P1
- [ ] **海棠堤** `beiyangyuan-haitang-bank` P1
- [ ] **牛顿苹果树** `beiyangyuan-newton-tree` P1
- [ ] **钦字第一号考凭** `beiyangyuan-qin-exam` P1
- [ ] **青年湖** `beiyangyuan-youth-lake` P1

### 服务
- [x] **大通学生中心** `beiyangyuan-datong-center` P1 ×2

### 其他
- [ ] **七星路** `beiyangyuan-qixing-road` P1
- [ ] **乐园** `beiyangyuan-le-garden` P3
- [ ] **书园** `beiyangyuan-shu-garden` P3
- [ ] **兴学路** `beiyangyuan-xingxue-road` P1
- [ ] **博文路** `beiyangyuan-bowen-road` P1
- [ ] **双台路** `beiyangyuan-shuangtai-road` P1
- [ ] **射园** `beiyangyuan-she-garden` P3
- [ ] **御园** `beiyangyuan-yu-garden` P3
- [ ] **数园** `beiyangyuan-shu-garden-2` P3
- [ ] **新元路** `beiyangyuan-xinyuan-road` P1
- [ ] **礼园** `beiyangyuan-li-garden` P3
- [ ] **西沽路** `beiyangyuan-xigu-road` P1

