...
export function PoiSources({poi}:{poi:POI}){
 ...
 return sources.length?<details><summary>介绍依据</summary>{sources.slice(0,3).map(s=><p key={s.id}><a href={s.url} target="_blank" rel="noreferrer">{s.title}</a> · {s.published_at??'发布日期未载明'}</p>)}</details>:null;
}
