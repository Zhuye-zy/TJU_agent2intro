...
function SourceDetails({ sources, final }: { sources: Source[]; final: boolean }) {
  ...
  return <details className="task-sources"><summary>{final ? '最终来源' : '检索候选来源'} {sources.length} 条</summary><ol>{sources.map((source, index) => { const href = safeSourceUrl(source.url); return <li key={source.id}><span>[{index + 1}]</span><div><strong>{source.title}</strong><p>{source.snippet}</p>{href && <a href={href} target="_blank" rel="noreferrer">打开资料</a>}</div></li>; })}</ol></details>;
}
...
export function App() {
  ...
  async function stopListening() { const id = asrRequestRef.current; asrRequestRef.current = null; ++asrGenerationRef.current; asrAbortRef.current?.abort(); asrAbortRef.current = null; setAsrBusy(false); setAvatarState('idle'); if (id) await asrRef.current?.stop(id); }
  ...
  return <div className="app-shell r2-shell">
    ...
    <main className="workspace r2-workspace" data-mobile-view={mobileView} style={{ '--panel-width': `${prefs.panelWidth}px` } as CSSProperties}>
      <section className="guide-stage"><CampusBackdrop campus={prefs.campus} assets={campusAssets}><div className="avatar-stage"><div className="campus-stamp"><span>AI4TJU</span><strong>{CAMPUS_NAMES[prefs.campus]}</strong></div><div className="avatar-host" ref={avatarHostRef} style={{ transform: `scale(${prefs.avatarScale})` }}/>{!avatarReady && <div className="avatar-fallback"><span className="fallback-monogram">珂</span><strong>{avatarMessage}</strong><p>不会使用替代人物冒充已有形象</p></div>}<div className={`avatar-state ${avatarState}`}><i/><div><small>珂莱塔</small><strong>{AVATAR_LABELS[avatarState]}</strong></div></div></div></CampusBackdrop>
        ...
        <CampusExplorer campus={prefs.campus} sessionId={currentSession('chat')} focusPoiId={focusPoiId} onSelect={onSelectPoi} onAssets={onCampusAssets} onReadRoute={(route) => { const requestId = freshUuid(); const task: TaskRecord = { ...newTask(requestId, freshUuid()), lane: 'chat', prompt: '路线讲解', mode: 'campus_qa', campus: prefs.campus, poiId: selectedPoiRef.current?.id ?? null, generation: null }; void playTask(task, routeNarration(route)); }} onAsk={(prompt, poi) => { selectedPoiRef.current = poi; setWorkView('chat'); setMobileView('work'); setChatInput(prompt); }}/>
      </section>
