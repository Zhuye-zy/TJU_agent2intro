...
interface Props {
  campus:CampusId; sessionId:string; focusPoiId:string|null;
...
function operationError(error:unknown):string {
  ...
  const labels:Record<string,string>={
    ...
    test_budget_exhausted:'本轮低额度预算已用完，请使用外部导航。',
...
export function CampusExplorer({campus,sessionId,focusPoiId,onSelect,onAssets,onAsk,onReadRoute}:Props) {
  ...
  const clearRoute=useCallback(()=>{routeScope.current.cancel();routeBusyRef.current=false;setRouteBusy(false);setRoute(null);setRouteError('');onlineRef.current?.clearRoute();},[]);
  const choose=useCallback((poi:POI|null)=>{selectedRef.current=poi;setSelected(poi);setExternalNav(null);clearRoute();},[clearRoute]);
  ...
  useEffect(()=>{
    let active=true;setAssets(null);onAssets(null);setZoom(1);clearRoute();locationScope.current.cancel();setLocating(false);
  ...
  useEffect(()=>{
    let active=true;
    void r2Transport.mapConfig().then(value=>{if(active)setMapConfig(value);}).catch(()=>{if(active)setOnlineError('在线地图配置暂不可用，基础导览不受影响。');});
    return()=>{active=false;};
  },[]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    try{budgetRef.current??=new MapBudget();updateBudget();}catch(error){setOnlineError(operationError(error));return;}
  ...
  useEffect(()=>()=>{routeScope.current.cancel();locationScope.current.cancel();},[]);
  ...
  async function beginLocation(){
    if(!onlineRef.current||locating)return;const operation=locationScope.current.begin();clearRoute();setPosition(null);setLocating(true);setLocationMessage('正在请求一次授权定位…');
    try{const result=await onlineRef.current.locate(freshUuid(),operation.controller.signal);if(operation.current()){setPosition(result);setLocationMessage('高德定位，精度约 '+Math.round(result.accuracy_m??0)+' 米。');}}
  ...
  function stopLocation(){locationScope.current.cancel();setLocating(false);setLocationMessage('已停止本地定位等待，不会持续追踪位置。');}
  function applyManualStart(){
    ...
    stopLocation();clearRoute();setPosition({lng,lat,crs:'GCJ02',source:'manual',accuracy_m:null,timestamp:new Date().toISOString()});setLocationMessage('手动起点（GCJ-02），不代表设备实际位置；2分钟内按需规划。');
  }
  async function planRoute(){
    const poi=selectedRef.current;if(!poi||poi.campus_id!==campus||!position||!onlineRef.current||routeBusyRef.current)return;
    ...
    try{
      const result=await onlineRef.current.walk({route_id:freshUuid(),session_id:sessionId,campus_id:campus,destination_poi_id:poi.id,entrance_id:null,origin:position,user_initiated:true},poi,operation.controller.signal);
