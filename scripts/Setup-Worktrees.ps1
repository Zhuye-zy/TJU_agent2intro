...
foreach ($item in $spec) {
 ...
 if (Test-Path -LiteralPath $path) {
  ...
  if ($top -ne $path -or $branch -ne $item.branch -or $head -ne $base) { throw "Existing worktree does not match baseline: $path" }
...
foreach ($job in $jobs) {
 ...
 $result += [pscustomobject]@{window=$job.spec.window;path=$job.path;branch=$job.spec.branch;base_commit=$base;web_port=$job.spec.web_port;api_port=$job.spec.api_port;install_exit_code=$code;env_copied=(Test-Path -LiteralPath (Join-Path $job.path '.env'));node_modules=(Test-Path -LiteralPath (Join-Path $job.path 'node_modules'));venv=(Test-Path -LiteralPath (Join-Path $job.path '.venv'))}
}
[pscustomobject]@{base_commit=$base;integration_branch='integration/m0';worktrees=$result} | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $root '.runtime\parallel-state.json') -Encoding UTF8
