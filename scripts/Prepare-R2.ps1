...
foreach($item in $spec){
 ...
 & (Join-Path $path 'scripts\Install.ps1') *> (Join-Path $runtime 'r2-install.log')
 ...
 Push-Location $path
 try {
  & npm.cmd run typecheck *> (Join-Path $runtime 'r2-typecheck.log')
 ...
 $result+=[pscustomobject]@{window=$item.window;path=$path;branch=$item.branch;base_commit=$base;web_port=$item.web_port;api_port=$item.api_port;install_exit_code=0;typecheck_exit_code=0;import_exit_code=0;env_copied=$false}
}
$state=Join-Path $root '.runtime\R2'
[IO.Directory]::CreateDirectory($state)|Out-Null
[pscustomobject]@{base_commit=$base;integration_branch='integration/m0';worktrees=$result}|ConvertTo-Json -Depth 5|Set-Content -LiteralPath (Join-Path $state 'parallel-state.json') -Encoding UTF8
