...
2026-09-15 更新：应用内步行支持自动 IP 起点、过期位置更新和未核验目的地的高德匹配，同名候选选择后自动继续。保留自动联网问答、末尾参考资料及四种导览语音。配置、行为及限制见 [联网与地图说明](docs/WEB_SEARCH_AND_MAPS.md)，地图当前状态以本机 `/api/maps/status` 为准。
...
## 开发和边界
`scripts/start-dev.ps1` 为5173/8000开发模式。前端所有模型请求只走自有后端，根 `.env` 和运行日志不会提交。

[第三方使用与许可](THIRD_PARTY_NOTICES.md)。角色和 SDK 授权不随代码许可证自动改变。
